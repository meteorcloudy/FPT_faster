...
inline void MergePair(PhylogenyTree *T1, PhylogenyTree *F2,TreeNode * n1, TreeNode * n2){
...
void FPTSolver:: MergeSiblingNodes(PhylogenyTree *T1, PhylogenyTree *F2){
...
Group FPTSolver:: FindGroup(PhylogenyTree * tree){
    ...
    while (!q.empty()){
        ...
        if (p->GetChild(0)->IsRtLeaf() && p->GetChild(1)->IsRtLeaf()) continue;
        ...
        for (int i=0;i<p->GetChildrenSize();i++)
            if (p->GetChild(i)->GetReflectId() != -1) {
                q.push(p->GetChild(i));
            }
    }
...
void FPTSolver:: MoveTree(PhylogenyTree *F, PhylogenyTree * T1, PhylogenyTree *F2){
    int pos = 0;
...
TreeNode * FPTSolver:: IsOneDistance(TreeNode *p1, TreeNode *p2){
    if (p1->GetParent()->GetParent() != NULL && ( p1->GetParent()->GetParent() == p2->GetParent() ) ) return p1->GetParent();
    if (p2->GetParent()->GetParent() != NULL && ( p1->GetParent() == p2->GetParent()->GetParent() ) ) return p2->GetParent();
...
bool FPTSolver:: Case_1_2_2_2(PhylogenyTree * F,PhylogenyTree * T1,PhylogenyTree * F2, int k, Group &grp2, vector<int> & pedant_ab){
    ...
    TreeNode * Rbc = F2->GetNodeById(lca.ask(grp2.c->GetId(), grp2.c->GetId()));
...
bool FPTSolver:: Case_2_3(PhylogenyTree * F,PhylogenyTree * T1,PhylogenyTree * F2, int k, Group &grp2, TreeNode * Rab){
    ...
    if (k >= pedant_ab.size()){
        ...
        if (Rab->IsRoot()){
            ...
            MergePair(_T1, _F2, _T1->GetNodeById(grp2.a->GetReflectId()), _T1->GetNodeById(grp2.b->GetReflectId()));
            ...
            if (Case_1_2_2_1(_F, _T1, _F2, k-(int)pedant_ab.size(), grp, pedant_cd)) return true;
        }
    }
    ...
    if (Case_1_2_2_1(_F, _T1, _F2, k-1, grp, pedant_cd)) return true;
...
bool FPTSolver:: Case_2_4(PhylogenyTree * F,PhylogenyTree * T1,PhylogenyTree * F2, int k, Group &grp2){
    ...
    if (k >= pedant_ab.size()){
        ...
        if (Case_1_2_2_2(_F, _T1, _F2, k-(int)pedant_ab.size(), grp, pedant_cd)) return true;
    }
    ...
    if (Case_1_2_2_2(_F, _T1, _F2, k-1, grp, pedant_cd)) return true;
...
bool FPTSolver:: MAF(PhylogenyTree * F,PhylogenyTree * T1,PhylogenyTree * F2, int k){
    ...
    // 合并所有能合并的叶节点
    MergeSiblingNodes(T1, F2);
    
    // 将合并完的子树从F2移动到F中
    MoveTree(F, T1, F2);
    
    if (F2->GetRootNum() == 0) { ans = new PhylogenyTree(F); clearParam(F, T1, F2); return true;}
